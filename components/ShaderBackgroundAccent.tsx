...
// Shader with conference theme colors - ACCENT VERSION
const vertexShader = `
  varying vec2 vUv;
  void main() {
    vUv = uv;
...
const fragmentShader = `
  ...
  // Conference theme colors - configured via conference.config.ts
  vec3 primaryColor = vec3(0.055, 0.098, 0.169); // Primary dark
  vec3 accentColor = vec3(0.992, 0.698, 0.275);  // Accent color
  vec3 creamColor = vec3(0.961, 0.941, 0.922);   // Light background
  ...
  vec4 cppn_fn(vec2 coordinate, float in0, float in1, float in2) {
    ...
    * buf[7]
    ...
    // Map to conference theme colors - ACCENT VERSION
    float pattern = buf[0].x * 1.5;
    float detail = buf[0].y * 1.2;
    
    vec3 baseColor = mix(creamColor, accentColor, pattern);
    vec3 finalColor = mix(baseColor, mix(accentColor, primaryColor, 0.4), detail);
...
function ShaderPlane() {
  ...
  return (
    <mesh ref={meshRef} position={[0, 0, -1]} scale={[3, 3, 1]}>
...
export default function ShaderBackgroundAccent() {
  const camera = useMemo(() => ({ position: [0, 0, 1] as [number, number, number], fov: 75, near: 0.1, far: 1000 }), []);
  
  return (
    <div className="absolute inset-0 w-full h-full" aria-hidden style={{ zIndex: 0 }}>
      <Canvas
        camera={camera}
